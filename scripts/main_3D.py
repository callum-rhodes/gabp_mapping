...
def init_ros():
    ...
    rospy.loginfo('Expected sensor frequency: {} Hz'.format(sensor_freq))
    ...
    resolution = rospy.get_param('~map_resolution', 1)
    resolution = float(resolution)
    rospy.loginfo('Output map resolution: {}m'.format(resolution))
    ...
    d_val = float(d_val)
    eps_val = rospy.get_param('~propagation_epsilon', 1e-2)
...
if __name__ == '__main__':
    try:
        [output_path, sensor_topic, sensor_freq, map_frame, sensor_child_frame, map_topic, resolution, freq_val, oct_update_freq, p_val, o_val, t_val, d_val, eps_val, metric, blank_bool, map_size] = init_ros()
        sensor_msg_type = Float32
        occ = maps.OccGenerator()
        if blank_bool:  # If using the blank map boolean initiates based on the requested size at [0,0,0]
            map_size = [int(float(x) / resolution) for x in map_size.split(",")]
            occ.blank2occ(map_size, resolution)
        else: #  Else will read the octomap message for the map
            ...
            occ.oct2occ(octree, resolution)  #  Converts the octomap msg into a 3D matrix of binary values
        ...
        for sens_num in range(0, len(sensor_topic_list)):
            gas_sensor.append(sensors.GasSensor(1, sensor_topic_list[sens_num], sensor_msg_type, map_frame, child_frame_list[sens_num]))  # Uses the sensors .py file to generate a historic gas sensor object
        ...
        #  Creates the GaBP object which stores all data
        GaBP_k = GaBP.G_GaBP_3D(occ, map_frame, lambda_p=p_val, lambda_o=o_val, lambda_d=d_val, epsilon=eps_val, lambda_t=t_val, distance=metric, pub_freq=freq_val, sens_freq=sensor_freq, filepath=output_path)
